...
const limits = [
  {
    key: "max_products",
    label: "Max Products",
    values: { Starter: 100, Growth: 1000, Enterprise: 10000 },
  },
  {
    key: "max_warehouses",
    label: "Max Warehouses",
    values: { Starter: 1, Growth: 5, Enterprise: 25 },
  },
  {
    key: "max_users",
    label: "Max Users",
    values: { Starter: 5, Growth: 25, Enterprise: 100 },
  },
  {
    key: "max_orders",
    label: "Max Orders/mo",
    values: { Starter: 500, Growth: 5000, Enterprise: 50000 },
  },
  {
    key: "max_websites",
    label: "Max Websites",
...
export default function PackageListPage() {
  ...
  const handleSave = async () => {
    ...
    setShowEdit(null);
    toast({
      title: showEdit ? "Package updated" : "Package created successfully",
    });
  };
    ...
    <div className="grid gap-4 py-4">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="space-y-1.5">
          ...
          <Input type="number" placeholder="1430" />
        </div>
      </div>
      <div className="space-y-1.5">
    ...
    <>
            ...
            <CardContent className="p-4">
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b">
                      <th className="text-left py-2.5 px-3 text-xs font-medium text-muted-foreground uppercase">
                        Module
                      </th>
                      {mockPackages.map((p) => (
                        <th
                          key={p.id}
                          className="text-center py-2.5 px-3 text-xs font-medium text-muted-foreground uppercase"
                        >
                          {p.name}
                        </th>
                      ))}
                          ...
                          <td key={p.id} className="py-2.5 px-3 text-center">
                            <Checkbox
                              ...
                              onCheckedChange={(checked) => {
                                ...
                                toast({
                                  title: `${m.name} ${checked ? "enabled" : "disabled"} for ${p.name}`,
                                });
                              }}
                            />
                          </td>
          ...
          <Card>
            <CardContent className="p-4">
                        ...
                        {mockPackages.map((p) => (
                          <td key={p.id} className="py-2.5 px-3 text-center">
                            <Input
                              ...
                              defaultValue={
                                l.values[p.name as keyof typeof l.values]
                              }
                            />
                          </td>
                        ))}
              ...
              <div className="mt-4 flex justify-end">
                <Button
                  onClick={() => toast({ title: "Limits saved successfully" })}
                >
                  Save Limits
                </Button>
              </div>
      ...
      >
        <DialogContent className="max-w-md max-h-[99vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>
              {showEdit ? "Edit Package" : "Create Package"}
            </DialogTitle>
          </DialogHeader>
          {formFields}
          <DialogFooter>
